  ...
  <div class="layout">
    ...
    <div class="all-container">
      ...
      <div class="container">
        <span
            class="list"
            v-for="(e,index) in notEmptyRocks"
            :key="index"
            :style="`
            zIndex: ${e?e.num:0};
...
export default {
  ...
  computed: {
    notEmptyRocks: function () {
      return this.rocks.filter(rocks => rocks);
    }
  },
  methods: {
    isMobile() {
    ...
    cssTransition(e) {
      if(this.isMobile()){
        return `${(e?e.x:0) * 12.2}vw,${(e?e.y:0) * 12.2}vw`
      }else if (this.smallWidth()) {
        return `${(e?e.x:0) * 23.7}vw,${(e?e.y:0) * 23.5}vw`
      } else {
